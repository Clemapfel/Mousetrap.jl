...
add_signal_descriptor(:close, void_signature, "popover is closed")
...
add_signal_descriptor(:key_pressed, "code::KeyCode, modifiers::ModifierState, [::Data_t]) -> should_invoke_default_handlers::Bool", "user pressed a non-modifier key")
...
add_signal_descriptor(:motion_enter, "x::AbstractFloat, y::AbstractFloat, [::Data_t]) -> Nothing", "once per frame while cursor is inside allocated area of the widget")
add_signal_descriptor(:motion_enter, void_signature, "cursor leaves allocated area of the widget")
...
add_signal_descriptor(:page_reordered, "page_index::Integer, [::Data_t]) -> Cvoid", "number of pages decreases while the widget is realized")
add_signal_descriptor(:page_reordered, "page_index::Integer, [::Data_t]) -> Cvoid", "currently visible page changes")
